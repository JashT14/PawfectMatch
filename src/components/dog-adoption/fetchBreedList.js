async function fetchBreedList() {
  try {
    ...
    const originalArrayBreeds = Object.entries(body.message); // breed and subbreed are separate strings
    ...
    originalArrayBreeds.forEach((item) => {
      ...
      if (subbreed.length === 0) {
        ...
        subbreed.forEach((subbreed) => {
          breedsArray.push(`${subbreed} ${breed}`); //combine breed and subbreed into one string
        });
      }
    });
